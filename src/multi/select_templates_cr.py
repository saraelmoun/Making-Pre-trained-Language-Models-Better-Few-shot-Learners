"""
ETAPE 8b — SELECTION des templates auto-generes (§5.2) sur CR (fichier NEUF, independant).

Lit 08a_templates.json, fine-tune chaque candidat sur seed 42 (classement sur val),
puis evalue le gagnant sur 5 seeds. Cible papier (Table 3, CR, auto) : 88.5.
...
def main():
    ...
    print(f"{len(templates)} templates CR a classer (seed {RANK_SEED}, {EPOCHS} epochs)")
    ...
    print(f"\n>>> MEILLEUR template CR : {best_tmpl}")
    ...
    result = {"step": "08_auto_template", "dataset": "CR (SetFit/SentEval-CR)",
    ...
    with open(os.path.join(OUT_DIR, "08_auto_template.txt"), "w") as f:
        ...
        f.write(f"Cible papier (Table 3, CR auto) : {PAPER_TARGET}\n")
        f.write(f"(rappel) manuel It was [MASK], nous : voir etape 04\n")

    print("\n" + "=" * 50)
    print(f"Template auto CR : {best_tmpl}")
    print(f"Prompt-based FT (AUTO template) : {mean:.1f} +/- {std:.1f}")
    print(f"Cible papier : {PAPER_TARGET} | manuel nous : voir etape 04")
    print("=" * 50)
