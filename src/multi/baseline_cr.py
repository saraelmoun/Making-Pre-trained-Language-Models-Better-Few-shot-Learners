"""
ETAPE 5 — Baseline FINE-TUNING CLASSIQUE sur CR (fichier NEUF, isole).
...
Meme protocole : 5 seeds x grille, selection sur val. Cible papier (Table 3, CR) : 76.9.
...
def main():
    ...
    print(f"Device : {device} | CR baseline classique | {len(SEEDS)} seeds")
    ...
    with open(os.path.join(OUT_DIR, "05_baseline.txt"), "w") as f:
        f.write("ETAPE 5 — BASELINE FINE-TUNING CLASSIQUE sur CR\n")
        f.write("=" * 50 + "\n")
    ...
    print(f"Fine-tuning CLASSIQUE CR : {mean:.1f} +/- {std:.1f}")
