"""
ETAPE 7 — Auto-selection des MOTS-LABELS (§5.1) sur CR (fichier NEUF, independant).

Le template est FIXE ("{sentence} It was [MASK] ."), on cherche les MOTS qui
representent le mieux chaque classe. Tache binaire (CR : 0=negatif, 1=positif).
...
N'importe QUE data_cr. Ne touche pas au code SST-2.
"""
...
def main():
    ...
    print(f"Device : {device} | CR §5.1 auto mots-labels | template fixe : {TEMPLATE}")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    model = AutoModelForMaskedLM.from_pretrained(MODEL_NAME).to(device)
    init_state = {k: v.detach().clone() for k, v in model.state_dict().items()}
    train, _, _ = load_cr_fewshot(k=16, seed=RANK_SEED)
    ...
    result = {"step": "07_auto_labelwords", "dataset": "CR (SetFit/SentEval-CR)",
    ...
    with open(os.path.join(OUT_DIR, "07_auto_labelwords.txt"), "w") as f:
        ...
        f.write(f"(rappel) manuel great/terrible, nous : voir etape 04\n")

    print("\n" + "=" * 50)
    print(f"Mots-labels auto CR : {{0:{best['class0_word']}, 1:{best['class1_word']}}}")
    print(f"Prompt-based FT (AUTO label words) : {mean:.1f} +/- {std:.1f}")
    print(f"(rappel) manuel, nous : voir etape 04")
    print("=" * 50)
