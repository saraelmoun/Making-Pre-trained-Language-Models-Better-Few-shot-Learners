"""
ETAPE 8a — Auto-generation des TEMPLATES avec T5 (§5.2) sur CR (fichier NEUF, independant).

Identique a la version SST-2 (T5-3B, beam joint eq.4, 2 variantes + fusion, decoupage memoire),
mais pointe sur data_cr. Mots-labels CR = {1:great, 0:terrible} (memes que SST-2, Table 1).
...
def main():
    ...
    print(f"Device : {device} | modele : {args.model} | beam : {args.beam} (CR)")
    tokenizer = AutoTokenizer.from_pretrained(args.model)
    model = T5ForConditionalGeneration.from_pretrained(args.model).to(device).eval()
    train, _, _ = load_cr_fewshot(k=16, seed=args.seed)
    print(f"Generation a partir de {len(train)} exemples CR (seed {args.seed})")
    ...
    print(f"\n{len(templates)} templates (CR) -> {OUT_DIR}/08a_templates.json / .txt")
