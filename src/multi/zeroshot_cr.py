"""
ETAPE 3 — Zero-shot prompt-based sur CR (fichier NEUF, isole).
...
SST-2, mais sur CR. Sauvegarde 03_zeroshot.json / .txt.
"""
...
def main():
    ...
    _, _, test = load_cr_fewshot(k=16, seed=42)
    print(f"Test set CR : {len(test)} phrases")
    ...
    with open(os.path.join(OUT_DIR, "03_zeroshot.txt"), "w") as f:
        f.write("ETAPE 3 — ZERO-SHOT prompt-based sur CR\n")
        f.write("=" * 40 + "\n")
    ...
    print(f"\nAccuracy ZERO-SHOT sur CR : {acc:.1f}%  ({correct}/{len(test)})")
