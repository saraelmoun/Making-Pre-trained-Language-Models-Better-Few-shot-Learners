"""
ETAPE 4 — Prompt-based FINE-TUNING (manuel) sur MR (fichier NEUF, isole).

Meme protocole que pour SST-2 : 5 seeds x grille (lr, batch), selection sur val
(departage par perte val), moyenne +/- ecart-type. Cible papier (Table 3, MR) : 87.0.
...
def main():
    ...
    print(f"Device : {device} | MR prompt-based FT manuel | {len(SEEDS)} seeds")
    ...
    with open(os.path.join(OUT_DIR, "04_prompt_ft_manual.txt"), "w") as f:
        f.write("ETAPE 4 — PROMPT-BASED FINE-TUNING (manuel) sur MR\n")
        f.write("=" * 50 + "\n")
    ...
    print(f"Prompt-based FT (manuel) MR : {mean:.1f} +/- {std:.1f}")
