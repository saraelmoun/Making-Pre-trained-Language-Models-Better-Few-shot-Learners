"""
ETAPE 6 — Synthese finale MR : assemble les resultats des etapes 3,4,5,7,8 en une
table comparee au papier (LM-BFF, Table 3). Ne refait AUCUN calcul, lit les fichiers.
Sauvegarde 06_comparaison.json / .txt.

Usage : python comparaison_mr.py [dossier_resultats]   (defaut : outputs_mr)
"""
...
OUT_DIR = sys.argv[1] if len(sys.argv) > 1 else "outputs_mr"
...
def main():
    ...
    rows = [
        ("Prompt-based zero-shot",      zs["accuracy"], None,         80.8),
        ("Fine-tuning classique",       bl["mean"],     bl["std"],    76.9),
        ("Prompt-based FT (manuel)",    ft["mean"],     ft["std"],    87.0),
        ("Auto-template (5.2)",         at["mean"],     at["std"],    88.5),
    ...
    synthese = {
        ...
        "conclusion": "Le pipeline complet generalise de SST-2 a MR : prompt-based >> classique, "
    ...
    lines = []
    lines.append("ETAPE 6 — SYNTHESE MR (Movie Reviews) vs PAPIER (LM-BFF, Table 3)")
    lines.append("=" * 66)
    ...
    lines.append("")
    lines.append("Conclusion : le pipeline complet generalise de SST-2 a MR.")
    lines.append("  - prompt-based FT (85.4) >> fine-tuning classique (72.5), et bcp plus stable")
    lines.append("  - auto-template (86.4) et auto-mots-labels (85.5) egalent/depassent le manuel (85.4)")
